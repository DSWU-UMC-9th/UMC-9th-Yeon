...
import { useCustomFetch } from "../hooks/useMovies";
...
export default function MovieDetail() {
  ...
  const {
    ...
    isLoading: loading,
    error: err,
  } = useCustomFetch<{ detail: MovieDetails; credits: Credits }>(
    async (signal) => {
      if (!movieId) throw new Error("영화 ID가 없습니다.");
      const [d, cr] = await Promise.all([
        fetchMovieDetails(movieId, { signal }),
        fetchMovieCredits(movieId, { signal }),
      ]);
      return { detail: d, credits: cr };
    },
    [movieId]
  );
